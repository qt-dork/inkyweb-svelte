// Almost entirely based off of:
// https://github.com/scpwiki/wikijump/blob/develop/web/modules/codemirror/src/editor-field.ts
...
export interface EditorFieldOpts<T> {
  ...
  /**
   * Function that runs when the view is updated. This does
   * not replace the `update` method the `StateField`
   * object is created with, instead this is ran after that
   * method determines the field's value.
   * 
   * If a value that isn't undefined is returned, that'll
   * replace the field's value.
   */
  update?: (value: T, transaction: Transaction, changed: boolean) => T | undefined | void

  /** Allows for providing values to facets, or even just
   * purely adding extensions.
   */
  provide?: (field: StateField<T>) => Extension

  /**
   * Function that, if given, will reconfigure a
   * `Compartment`with the returned `Extension` when this
   * field updates. Return null to indicate no extensions,
   * return false to indicate that the extensions should
   * not actually be reconfigured.
   */
  reconfigure?: (value: T, last: T | null) => Extension | null | false
}

/**
 * Smart handler for adding fields to CodeMirror
 * editor instances.
 * 
...
export class EditorField<T> {
  /**
   * The `StateEffect` for the field. This is a unique
   * object that is solely capable of modifying the
   * field's value.
   */
  private declare effect

  /**
   * The `StateField` for the field. This is a object that
   * describes the behavior of the field, such as how it is
   * created or updated.
   */
  ...
  /**
   * The extension that mounts this field to an editor.
   * Additionally, providing this field allows for simply
   * treating any instance of an `EditorField` as an
   * extension
   */
  declare extension: Extension

  /**
   * A mapping of `EditorView` objects to observables, for
   * the purpose of tracking their existence and updating
   * them.
   */
  ...
  /**
   * Returns an extension that mounts this field, but using
   * a different creation value.
   * 
  ...
  /**
   * Returns a Svelte-compatible observable for reactively
   * reading and updating the field. If a observable
   * already exists for the view given, it'll simply be
   * reused. This means it is safe to call this method
   * repeatedly for a view.
   */
  bind(view: EditorView): Writable<T> {
    if (this.observableMap.has(view)) return this.observableMap.get(view)

    // creates an observer that automatically adds and
    ...
    return {
      ...
      // so we don't want to double update by informing them again here
