import type { JSONObject, JSONValue } from "@sveltejs/kit/types/internal";
import { get_binding_group_value } from "svelte/internal";
import { writable, type Updater, type Writable } from "svelte/store";

/**
 * `localStorage`-based smart preference handler,
 * with support for observables or proxied objects.
 */
export class PreferenceHandler {
  ...
  /**
   * Returns the name given with a prefix namespace, to
   * prevent collisions with other items in `localStorage`.
   */
   ...
   * If that isn't found, the fallback value will instead
   * be returned.
   * 
   * @param name - The name/key for the preference.
   * @param fallback - The default value to return if the
  ...
  /**
   * Sets the preference with the given name to the given
   * value. PAssing an empty string will remove the
   * preference from storage.
   * 
   * The `value` given **must** be in such a format that it
   * can be stringified by `JSON.stringify`. Otherwise, data *will*
   * be lost.
   * 
   * @param name - The name/key for the preference.
   * @param value - The value to set. An empty string
  ...
  stored() {
    ...
    for (let idx = 0; idx < len; idx++) {
      ...
      if (name?.startsWith(this.prefix)) {
        keys.add(name.substring(this.prefix.length))
      }
    }
    return [...keys]
  }
  ...
  /**
   * Returns a writable observable store that maps to the
   * given preference. Works with deep accesses and writes.
   * 
   * @example
   * 
   * ```svelte
  ...
  /**
   * Retrieves a 'wrapped' proxy object, or creates one if
   * needed. Setting items on this object will automatically
   * cause the object to be stored. This can be used to store
   * a record of preferences without needing to use an
   * observable store.
   * 
   * @example
   * 
  ...
  wrap<T extends JSONObject>(name: string, fallback: T): T {
    ...
    const handler: ProxyHandler<T> = {
      ...
      // fire setter function whenever the object has a 
      // property set (even recursively)
      set: (target, prop, val) => {
...
/**
 * Pre-made {@link PreferenceHandler} with the default
 * `"_user-pref_"` prefix set.
 * 
